...
- **Antigravity (Google) subscription**: store `ANTIGRAVITY_ACCOUNTS_JSON` credential (contents of `~/.config/opencode/antigravity-accounts.json` after an `opencode auth login` against the `opencode-antigravity-auth` plugin). CASCADE writes the file before each run and persists refreshed tokens back to the DB after. The `antigravity` engine runs on the OpenCode server with the pinned `opencode-antigravity-auth` plugin (see `ANTIGRAVITY_PLUGIN_VERSION` in `src/backends/antigravity/index.ts`, kept in lockstep with the `Dockerfile.worker` install). The model list lives in `src/backends/antigravity/models.ts` (the plugin's Antigravity-quota models: `gemini-3-pro`, `gemini-3.1-pro`, `gemini-3-flash`, `claude-sonnet-4-6`, `claude-opus-4-6-thinking`), addressed as `google/antigravity-<model>` (default `google/antigravity-gemini-3-pro`). There is no "Gemini 3.5" model — Antigravity's Flash model is `gemini-3-flash`. Keep the model specs in sync with the pinned plugin version's model catalog. Reasoning effort is the optional `reasoningEffort` engine setting: rather than OpenCode's runtime `--variant` (unavailable on the pinned `opencode-ai@1.14.25` SDK), it is injected into the selected model's `options` in `getConfigOverrides` (`thinkingLevel` for Gemini, `thinkingConfig.thinkingBudget` for Claude `*-thinking`), which the plugin reads via `providerOptions.google`.
